...
class Block(nn.Module):
    ...
    def forward(self, x, get_att=False, index = 11):
        x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        if index == 11 and x.shape[0] > 3:
            norm_x = self.norm2(x)
            x = x + self.drop_path(self.smoe(norm_x.float().transpose(0, 1)).transpose(0, 1))
...
class Trans(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768,
                 ...
                 view=0,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu=1.0, miss_type = "nothing"):
        super().__init__()
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        for index, blk in enumerate(self.blocks):
            x = blk(x, index)
            if getattr(self.blocks[index], 'aux_loss') is not None and self.training:
                loss_aux_list = loss_aux_list + self.blocks[index].aux_loss
            if B > 3:
                loss_aux_list = loss_aux_list + (self.gfmoe.l_aux + self.gfmoe_2.l_aux) * self.aux_loss_weights

        return self.norm(x), loss_aux_list

    def forward(self, x, cam_label=None, view_label=None):
        x, loss_aux_list = self.forward_features(x, cam_label, view_label)
        return x, loss_aux_list
